---
title: "Data Analysis - second Experiment"
...
Files <- list.files(here("Data", "Results_Exp2", "190430_mix2_QC_normalised_absolute_batch"))

File_list <- 
lapply(Files, function(x) {
  temp <- read_csv(here("Data", "Results_Exp2", "190430_mix2_QC_normalised_absolute_batch", x))
  name <- sub("^.+_(\\w+\\d)_.+","\\1", x)
  Name <- paste(toupper(substr(name,1,1)), substr(name,2,nchar(name)), sep ="")
  tempm <- mutate(temp,
                  Plate = Name,
                  Antibiotic = substr(Name, 1, nchar(Name)-1),
...
Data_poly %>% 
  left_join(Expected) %>% 
  mutate(Average =  expected_mean - GrowthYield ,
         Dominance = expected_max - GrowthYield,
         Weighted = expected_wmean - GrowthYield) %>% 
