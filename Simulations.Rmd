...
if(Scenario == 3) { #weighted mean
  save("Fig_2_a", file = here("figures", "Fig_2_a.RData"))
    ...
    Fig_S3_a <- Fig_2_a
  save("Fig_S3_a", file = here("figures", "Fig_S3_a.RData"))
    } else if(Scenario == 1) { #dominance
      Fig_S3_d <- Fig_2_a
    save("Fig_S3_d", file = here("figures", "Fig_S3_d.RData"))}
...
if(Scenario == 3) { #weighted mean
  ...
  # } else if(Scenario == 2) { #mean
  #   Fig_S3_b <- Fig_2_b
  # save("Fig_S3_b", file = here("figures", "Fig_S3_b.RData"))
  #   } else if(Scenario == 1) { #dominance
  #     Fig_S3_e <- Fig_2_b
  #   save("Fig_S3_e", file = here("figures", "Fig_S3_e.RData"))}
...
###Fig S2 - slope histograms
```{r}
p <- 
Results %>% 
  mutate(metric = factor(metric,  labels = c("beta slopes", "transgressive\noveryielding"))) %>% 
  ggplot(aes(x = slope, fill = metric))+
...
if(Scenario == 3){
  ggsave(plot = p, filename = here("figures", "Fig_S2.pdf"), width = 6, height = 3)
}
...
Results %>% 
  filter(metric == "beta_slopes") %>% 
  ggplot(aes(x = slope))+
  geom_histogram(binwidth = 0.001, fill = col_2ai)+
  geom_vline(xintercept = 0, linetype = "dashed")+
...
# if(Scenario == 3){
#   save("Fig_2_a_i", file = here("figures", "Fig_2_a(i).RData"))
...
```{r}
newdata = tibble(envnum = seq(1,5,0.1))

Res_slopes_trans <- 
Results %>% 
  filter(metric == "beta_slopes") %>% 
  pull(value) %>% 
  map(~predict.lm(., newdata = newdata)) %>% 
  map_df(~tibble(value = .)) %>% 
  mutate(envnum = rep(newdata$envnum, nrow(Results)/2)) %>% 
  mutate(rep = rep(1:(nrow(Results)/2), each = nrow(newdata))) 

med_slope <- 
Results %>% 
  select(-value) %>% 
  filter(metric == "beta_slopes") %>% 
  slice(1:999) %>% #has to be odd number to find exact median
  mutate(rowid = 1:n()) %>% 
  filter(abs(slope - median(slope)) < 10e-10) %>% 
  pull(rowid)

ggplot(Res_slopes_trans, aes(x = envnum, y = value, group = rep))+
  geom_line(size = 0.5, alpha = 0.1)+
  geom_line(data = . %>% filter(rep == med_slope), colour = "darkred", size = 1)+
  theme_classic()+
...
Results %>% 
  filter(metric == "transgressive") %>% 
  ggplot(aes(x = slope))+
  geom_histogram(binwidth = 0.006, fill = col_2bi)+
  geom_vline(xintercept = 0, linetype = "dashed")+
...
# 
# if(Scenario == 3){
#   save("Fig_2_b_i", file = here("figures", "Fig_2_b(i).RData"))
...
##Fig S11 - species specialisation
...
tibble(spec_spec = Species_specialisation, 
       transgressive = Results$transgressive)
...
if(Scenario == 3){
ggsave(filename = here("figures", "Fig_S11.pdf"), width = 6, height = 4)
}
