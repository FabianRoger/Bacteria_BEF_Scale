...
env_metrics_list <- 
lapply(env_metrics_list_oy, function(x){
  x %>% 
  select(-data) %>% 
  gather(metric, value, -envnum, -env_comb) %>%
  ungroup() %>% 
  nest(data = -metric) %>% 
  mutate(lm = map(data, ~lm(value ~ envnum, data = .x))) %>% 
  select(-data)
})
...
Results <- 
 bind_rows(env_metrics_list) %>% 
  mutate(slope = map_dbl(map(.$lm, coef), "envnum")) %>% 
...
newdata = tibble(rep = rep(1:(nrow(Results)/3), each = length(env_pred)),
                 env_pred = rep(env_pred, (nrow(Results)/3)))
...
Fig_2a <- 
ggplot(Results_pred, aes(x = env_pred, y = predict, group = paste(rep, metric), colour = metric))+
